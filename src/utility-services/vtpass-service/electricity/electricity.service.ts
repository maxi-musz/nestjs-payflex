  ...
  async purchase(userPayload: any, dto: PurchaseElectricityDto) {
    ...
    if (existingTx) {
      if (existingTx.status === 'success') {
        ...
        const token = (existingTx.meta_data as any)?.electricity_token;
        return new ApiResponseDto(true, 'Electricity purchase already completed', {
          ...(cachedResponse || { requestId: request_id }),
          ...(token ? { electricity_token: token } : {}),
        });
      }
      const cachedResponse = (existingTx.meta_data as any)?.vtpass_response;
    ...
    return this.orchestrator.executePurchase({
      ...
      auditAction: 'ELECTRICITY_PURCHASE',
      auditFailAction: 'ELECTRICITY_PURCHASE_FAILED',
      cashbackServiceType: 'electricity',
      ...
      onSuccess: async (vtpassResponse, txRecord) => {
        try {
          ...
          this.logger.error(`Failed to send electricity success email: ${e.message}`);
        }
      },
      onEnrichResponse: (vtpassResponse, extraMeta) => {
        return extraMeta.electricity_token ? { electricity_token: extraMeta.electricity_token } : {};
      },
    });
  }
