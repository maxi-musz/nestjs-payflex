  ...
  async purchase(userPayload: any, dto: PurchaseEducationDto) {
    ...
    try {
      ...
      createdTx = await this.prisma.$transaction(async (tx) => {
        ...
        if (!wallet || Number(wallet.current_balance) < resolvedAmount) {
      ...
      if (shouldRefund) {
        await this.prisma.wallet.update({
          ...
          data: { current_balance: { increment: resolvedAmount } },
        });
        this.logger.log(`Refunded ₦${resolvedAmount} to wallet for user ${userPayload.sub}`);
        if (shouldThrow) {
