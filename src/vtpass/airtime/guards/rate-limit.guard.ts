...
export class RateLimitGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    const windowSeconds = Number(this.config.get('AIRTIME_RATE_WINDOW_SECONDS') || 60);
    const maxRequests = Number(this.config.get('AIRTIME_RATE_MAX_REQUESTS') || 5);
    const ipWindowSeconds = Number(this.config.get('AIRTIME_IP_RATE_WINDOW_SECONDS') || windowSeconds);
    const ipMaxRequests = Number(this.config.get('AIRTIME_IP_RATE_MAX_REQUESTS') || maxRequests * 2);
